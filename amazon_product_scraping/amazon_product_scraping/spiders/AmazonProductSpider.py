...
class AmazonProductSpider(scrapy.Spider):
    ...
    start_urls = ["https://amazon.in/dp/B00CQ41JE4"]
    ...
    def parse(self, response):
        ...
        offers = (
            response.xpath('//span[@class="saving-prompt"]/text()').extract() or "NA"
        )
        original_price = (
            response.xpath(
        ...
        fullfilled = (
            response.xpath('//span[@class="a-icon-text-fba"]/text()').extract_first()
            or "NA"
        )
        ...
        details = response.xpath(
            '//div[@id="detailBullets_feature_div"]//ul[@class="a-unordered-list a-nostyle a-vertical a-spacing-none detail-bullet-list"]/text()'
        ).extract()
        ...
        subscription_discount = (
            response.xpath(
                '//tr[@id="regularprice_savings"]//td[@class="a-span12 a-color-price a-size-base priceBlockSavingsString"]/text()'
            ).extract_first()
            or "NA"
        )
